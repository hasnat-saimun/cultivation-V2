...
foreach ($commonNames as $name) {
    if (!isset($idByName[$name])) {
        echo json_encode(['error' => 'missing common subject', 'name' => $name]);
        return;
    }
    $commonIds[] = $idByName[$name];
}
if ($religionId > 0) {
    $commonIds[] = $religionId;
}
...
if ($religionId > 0) {
    $commonSort[$religionId] = 80;
}

$scienceNames = ['Physics-136', 'Chemistry-137', 'Biology-138'];
$scienceIds = [];
foreach ($scienceNames as $name) {
    if (!isset($idByName[$name])) {
        echo json_encode(['error' => 'missing science subject', 'name' => $name]);
        return;
    }
    $scienceIds[] = $idByName[$name];
}
$scienceSort = [
...
$saveCommon = Request::create('/result/curriculum-mapping/save', 'POST', $scopeCommon + [
    'subjectIds' => $commonIds,
    'sortOrders' => $commonSort,
]);
$saveCommon->setLaravelSession(app('session')->driver());
$controller->save($saveCommon);

$saveScience = Request::create('/result/curriculum-mapping/save', 'POST', $scopeScience + [
    'subjectIds' => $scienceIds,
    'sortOrders' => $scienceSort,
]);
$saveScience->setLaravelSession(app('session')->driver());
$controller->save($saveScience);
...
$rows = CurriculumSubjectMapping::query()
    ...
    ->where(function ($query) use ($student) {
        $query->where('section_id', (string) $student->sectionName)
            ->orWhereNull('section_id');
    })
    ->where(function ($query) use ($student) {
        $query->where('department_id', (string) $student->departmentName)
