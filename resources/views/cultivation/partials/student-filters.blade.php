@php
    // Display protection for rolling deployments; controllers must still provide
    // the canonical payload through StudentFilterService::viewPayload().
    ...
    $filters = $filters ?? [];
    $filterAction = $filterAction ?? ($action ?? url()->current());
    $filterResetUrl = $filterResetUrl ?? $filterAction;
@endphp
<form method="GET" action="{{ $filterAction }}" class="row g-2 align-items-end student-filter-form">
    ...
    <div class="col-auto"><button type="submit" class="btn btn-primary">Filter</button><a href="{{ $filterResetUrl }}" class="btn btn-light">Reset</a></div>
</form>
@once
@push('scripts')
<script>
