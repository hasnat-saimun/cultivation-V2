...
class PlacementPreviewTest extends TestCase
{
    ...
    private function scope(int $passing=2): array {$session=new sessionManage();$session->session='2026';$session->save();$class=new classManage();$class->className='Class 10';$class->save();$section=new sectionManage();$section->section='A';$section->save();$exam=new Exam();$exam->examName='Annual';$exam->passingSystem=$passing;$exam->save();return compact('session','class','section','exam');}
    ...
    private function mark(newAdmission $student,array $scope,Subject $subject,float $cq,float $gp,?float $mcq=null,?float $pr=null,?Exam $exam=null):Marksheet{return Marksheet::create(['studentId'=>$student->id,'classId'=>$scope['class']->id,'sessionId'=>$scope['session']->id,'groupId'=>$scope['section']->id,'examId'=>($exam??$scope['exam'])->id,'subjectId'=>$subject->id,'subjectMarks'=>$cq,'objectMarks'=>$mcq,'practicalMarks'=>$pr,'totalMarks'=>$cq+($mcq??0)+($pr??0),'gradePoint'=>$gp,'laterGrade'=>$gp>0?'Stored':'F']);}
