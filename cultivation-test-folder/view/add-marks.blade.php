...
<script>
    document.addEventListener('DOMContentLoaded', function(){
        ...
        if(!classSelect || !sectionSelect || !optionalGroupSelect || !subjectSelect){
            return;
        }
        ...
        function setSubjectLoadingState(message){
            ...
            try{
                if(window.jQuery){
                    window.jQuery(subjectSelect).trigger('change.select2');
                }
            }catch(e){ /* ignore */ }
        }

        function setSubjectEmptyState(){
        ...
        async function loadSubjects(){
            ...
            try{
                const res = await fetch(subjectEndpoint, {
                    method: 'POST',
        ...
        function resetDependentSelectorsForClassChange(){
            sectionSelect.value = '';
            optionalGroupSelect.value = '';

            try{
                if(window.jQuery){
                    window.jQuery(sectionSelect).trigger('change.select2');
                    window.jQuery(optionalGroupSelect).trigger('change.select2');
                }
            }catch(e){ /* ignore */ }
        }

        function onSelectionChanged(){
            resetDependentSelectorsForClassChange();
            syncOptionalGroupVisibility();
            loadSubjects();
        }

        // Use delegated handlers so select bindings survive any plugin-driven element re-init.
        document.addEventListener('change', function(event){
            if(event.target && event.target.id === 'marks_class_select'){
                onSelectionChanged();
            }
            if(event.target && (event.target.id === 'marks_section_select' || event.target.id === 'marks_optional_group_select')){
                loadSubjects();
            }
        });
